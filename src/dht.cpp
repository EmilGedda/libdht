...
namespace dht {
  ...
  auto device::poll() -> response {
    ...
    while(!valid_crc) {
      ...
      if (!valid_crc) {
        std::cerr << "Invalid CRC reading: got " << std::hex << crc << ", expected " << checksum << '\n';
      }
    }
  ...
  auto device::read_data() -> std::bitset<response_bitcount> {
    ...
    // Next it will send 40 bits of Data. Each bit starts with a 50µs LOW followed by HIGH for 26-28µs for a “0” or for 70µs for a “1”.
